...
### 7.6a 投影语义限制与 S12（二轮评审发现 F-8，2026-07-09）

- **受限表达式语言不能表达"存在同时满足 A 与 B 的元素"**：`any(xs[].a == 1) and any(xs[].b == 2)`
  是两次独立存在判断，不是同元素合取；而 `any(xs[].a == 1 and xs[].b == 2)` 在当前求值器下
  语义错误（列表参与 and 按"非空"求真）。**规则：涉及同元素多条件的判断必须由解析器产出
  派生标量字段**（如 `inconsistent_ports`、`deny_hit_count`），表达式只比较标量。
- **S12（下一轮实施）**：静态检测"投影未经聚合函数包裹即参与 and/or/比较另一投影"→ ERROR。
  语言级扩展（如二元 `any(xs[], cond)`）暂不引入——保持弱模型可执行的简单性优先（R7）。

### 7.6b verify.assert 字段契约（裁决 R-5 方向）

- assert 引用的"健康类字段"（service_active/mount_rw/rollout_succeeded/unready_pods 等）
  属**解析器输出契约**：解析器注册时必须声明其输出字段清单；`*_before` 后缀字段属
  **引擎快照契约**（action 执行前自动采样 verify.run 并存基线）。
- 待解析器注册表带字段声明后，新增校验："assert 引用的字段必须在对应解析器声明的输出
  或引擎快照约定内"。实施排期见 TODO 第三轮。

### 7.6c 节点标量输出的模板引用（裁决 R-7，2026-07-09 三轮）

- 检查节点解析器产出的**标量字段**（如 max_query_time、seconds_behind）在模板中经
  **`{{output.<scalar>}}`** 引用——`output` 已是合法节点输出根（§7.4），语义扩展为
  "当前节点解析结果的标量命名空间"。渲染引擎按该节点 parser 的 `scalars` 声明解析；
  引用未声明的标量按 S9/FIELD 处理。
- 实施（第四轮 T-3）：渲染引擎将 parser 标量并入 output.*；FIELD 校验同步覆盖模板引用。

### 7.6d S13 求值冒烟（四轮评审裁决，or-bug 启示）

- 每个 `branch.when` 与 `verify.assert` 除语法(S5)与投影(S12)校验外，**必须在空上下文上
  实际通过求值器执行一次**（缺字段→None 属正常；解析器/求值器的结构性异常→ERROR）。
- 动机：or 短路 bug 证明存在"语法校验通过、运行时才崩"的求值器缺陷类；S13 把这类缺陷
  拦在入库前。已实现于 validate.py。
